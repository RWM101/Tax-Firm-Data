...
EmployeeCTE AS(
SELECT ns.NewStaffName AS Name, NAME AS OldName, coalesce(FTEAmount,0) AS FTEAmount, spm.STATUS AS EmployeeStatus, spm.SKILLLEVEL, scs.Description, scs.Level, FTECTE.NewNameFull,
...
WHERE EmployeeStatus = "Active" AND spm.SKILLLEVEL IN ("Intern", "Associate") AND ns.NewStaffName NOT IN ("Ward, Issac", "Hoover, Vance", "Rich, Laura", "Ingram, Bishop")
GROUP BY OldName
ORDER BY Subtotal
)
